...
The MCP Worker also runs these suites on a Cloudflare schedule (issue #291), inside a Workflow rather than by a person at a keyboard. `tier` runs daily, at 05:52 UTC. `fit`, `chat` and `leak` run weekly, Sundays at 07:07 UTC. The split exists because the two halves cost differently: `tier` needs no token and spends no inference, so a daily run costs nothing beyond one D1 write, while `fit`, `chat` and `leak` are fifteen paced, inference-backed calls through AI Gateway, and running that set every day would spend real money against a corpus and a set of models that do not move on a daily rhythm.

The Sunday run sits 95 minutes behind the corpus refresh at 05:32 UTC, and that gap is slack rather than a measurement. `fit` and `chat` are graded against the Vectorize index the refresh upserts, a Vectorize upsert is applied asynchronously, and a case with a `min_sources` expectation that queries a still-applying index goes red with nothing behind it. Most Sundays the refresh is close to instant, because the corpus job is incremental; the Sunday after new content lands is the one where it is not, and that is the Sunday the schedule exists for. Nothing in this repository records how long a non-trivial refresh takes or how long the index takes to reflect one, so the number is wide on purpose. Timing a refresh that embeds a real batch, then polling the index for the last upserted id until it answers, is what would replace the slack with a figure.
...
A scheduled run can be started on demand, which is what a red Sunday usually wants. The params are a positional JSON string rather than a flag, and the config has to be the MCP Worker's, because the repository root holds the site's:

```bash
npx wrangler workflows trigger rlme-evals '{"suites":["tier"]}' -c workers/mcp/wrangler.jsonc
npx wrangler workflows instances describe rlme-evals latest -c workers/mcp/wrangler.jsonc
```

Name the suites explicitly. `suites` has to be an array, and every name in it has to be one of `tier`, `fit`, `chat` or `leak`: triggering with no params at all, or with a name that is not a suite, fails the instance before it mints anything and says which of the two happened. An empty array is accepted and asks for nothing. The suites run in the order they are listed, so put `leak` last for the same reason the manual runner does.

This costs real inference for `fit`, `chat` and `leak`, through the same shared quota and the same pacing as `npm run evals`. It is not free and it is not instant: a full weekly set is fifteen cases with twelve twenty-five-second gaps between them.
...
`eval_runs` carries a `status` column now, `ran` or `incomplete`. A row that could not run at all, because the mint failed, a suite threw before finishing, or every case failed with `unreachable` before the model said anything, records `incomplete` rather than going unwritten or reading as zero passed, and `/ops` reads it that way: its Graded column reads "did not run" and its Couldn't run column reads "not recorded," in place of numbers that would otherwise read as a real pass rate. A row that did run carries an `unreached` count as well (migrations/0008): cases whose model call never came back, kept out of `total`, `passed` and `failed` and shown in the Couldn't run column rather than as graded failures. `EVALS_RUNNER` is the override variable that turns the scheduled run off; it joins the list named in `CLAUDE.md`'s Tests section, off only in the test harness and never in a deployed config.
