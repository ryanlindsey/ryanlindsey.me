...
The MCP Worker also runs these suites on a Cloudflare schedule (issue #291), inside a Workflow rather than by a person at a keyboard. `tier` runs daily, at 05:52 UTC. `fit`, `chat` and `leak` run weekly, Mondays at 07:07 UTC. The split exists because the two halves cost differently: `tier` needs no token and spends no inference, so a daily run costs nothing beyond one D1 write, while `fit`, `chat` and `leak` are fifteen paced, inference-backed calls through AI Gateway, and running that set every day would spend real money against a corpus and a set of models that do not move on a daily rhythm.

The Monday run sits 95 minutes behind the corpus refresh at 05:32 UTC, and that gap is slack rather than a measurement. `fit` and `chat` are graded against the Vectorize index the refresh upserts, a Vectorize upsert is applied asynchronously, and a case with a `min_sources` expectation that queries a still-applying index goes red with nothing behind it. Most Mondays the refresh is close to instant, because the corpus job is incremental; the Monday after new content lands is the one where it is not, and that is the Monday the schedule exists for. Nothing in this repository records how long a non-trivial refresh takes or how long the index takes to reflect one, so the number is wide on purpose. Timing a refresh that embeds a real batch, then polling the index for the last upserted id until it answers, is what would replace the slack with a figure.
...
A scheduled run can be started on demand, which is what a red Monday usually wants. The params are a positional JSON string rather than a flag, and the config has to be the MCP Worker's, because the repository root holds the site's:
