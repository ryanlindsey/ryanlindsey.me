...
import { callAnalyzeFit, type McpClientEnv } from '../src/lib/fit/client';
...
test('reads the result through the keep-alive frames of a long call', async () => {
  ...
  const outcome = await callAnalyzeFit(envReturning(body), 'token', 'a description');
...
test('skips a notification frame that arrives before the response', async () => {
  ...
  // line, which belongs to a notification carrying no `result`. That lands on
  // `body.error || result === undefined` in `callAnalyzeFit`, which answers
  // `unreachable` and logs NOTHING. Fixing only the keep-alive prefix would
  // leave this standing.
  const notification = {
    jsonrpc: '2.0',
...
test('reads a plain JSON body, the other response mode Streamable HTTP defines', async () => {
  ...
  const outcome = await callAnalyzeFit(envReturningUntyped(body), 'token', 'a description');
