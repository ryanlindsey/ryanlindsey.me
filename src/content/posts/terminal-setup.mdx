...
standfirst: 'Ghostty, zsh, the Rust CLI replacements and a Claude Code setup tuned for reviewing agent work you were not present for.'
...
One thing I have deliberately left out. If you install from or publish to a private registry you will need authentication, and the tempting move is to export a token into your shell environment so it is always there. Do not do that, or at least do not do it without deciding to. Every process you start inherits your environment, including every command an agent runs, and a token that is always present is a token that is present during the mistake. Keep the credential in a secrets manager and supply it to the single command that needs it.
...
Claude Code loads plugins from marketplaces you name yourself. These are mine, and the keys belong in the same `~/.claude/settings.json` as above, merged into it rather than replacing it:
...
- Typing a command you have run before shows a gray completion after the cursor. That is `zsh-autosuggestions`, and it means Sheldon is loading.
