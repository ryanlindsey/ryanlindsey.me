...
description: 'An agent can read every page of this site as markdown, call the resume, the case studies and a retrieval-backed search as Model Context Protocol tools, and present a scoped token for what is not public. Underneath sits most of the Cloudflare developer platform: D1, KV, R2, Queues, Durable Objects, Vectorize, Workers AI, Browser Rendering, Analytics Engine and crons. Nearly all of it was written by agents working a plan. This is the build log, including the checks that were green about the wrong thing.'
