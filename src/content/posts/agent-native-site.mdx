...
description: 'An agent can read every page of this site as markdown, call the resume, the case studies and a retrieval-backed search as Model Context Protocol tools, and present a scoped token for what is not public. Underneath sits most of the Cloudflare developer platform: D1, KV, R2, Queues, Durable Objects, Vectorize, Workers AI, Analytics Engine and crons. Nearly all of it was written by agents working a plan. This is the build log, including the checks that were green about the wrong thing.'
standfirst: 'Every page as markdown, the résumé and case studies as MCP tools, and most of Cloudflare underneath, written by agents working a plan.'
...
`ryanlindsey-me` serves the site: static assets, the on-demand routes, a queue consumer, and a daily cron that runs the transcript retention sweep. `ryanlindsey-me-mcp` serves the protocol endpoint, the chat backend, the fit engine, the model judge, a rate limiter Durable Object, and its own crons, which refresh the retrieval corpus and run the evals on a schedule.
...
The suite runs against real Workers inside workerd rather than against mocks, with the site Worker booted from the adapter's build output, so the tests exercise the artifact that ships. That property is the part worth stating, because the count moves every week and the property does not: 791 tests across 52 files when this post went up, and 1,072 across 66 files at last measurement, one of them skipped.
