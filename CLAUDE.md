...
`ryanlindsey-me-mcp` (`workers/mcp/wrangler.jsonc`) serves the MCP protocol, the chat endpoint, the fit engine, the LLM judge, the rate limiter Durable Object, and three crons: 05:32 UTC refreshes the Vectorize corpus, 05:52 UTC runs the `tier` eval suite daily, and 07:07 UTC on Mondays runs `fit`, `chat` and `leak`, the three suites that spend inference (issue #291). The Monday gap behind the corpus refresh is 95 minutes of slack rather than a measured number: two of those suites are graded against the index that job upserts, Vectorize applies an upsert asynchronously, and nothing here has timed either. `scheduled()` branches on which cron fired, so the corpus refresh now runs only for its own expression, a change from running for every trigger when this Worker had only one.
...
Share cards are rendered by an `astro:build:done` hook in Node (`src/lib/og/integration.ts`), not by a route, because the adapter prerenders in workerd and native resvg cannot load there (#363). Every card URL carries a hash of what the card says, which is what lets `/og/*` be cached for a year. A change to how a card is drawn, rather than to what it says, moves no hash on its own: bump `OG_CARD_CONTRACT_VERSION` in `src/lib/og/cards.ts` in the same change, or every scraper keeps the old image. A change to `src/styles/tokens.css` or to the card fonts `src/lib/og/render.ts` loads is a drawing change too, even though neither file is named on an `OgCard`, and needs the same bump. `tests/og-cards.test.ts` now enforces the bump mechanically, by comparing a fingerprint of the palette, the fonts, the installed render libraries and `src/lib/og/layout.ts` against `OG_CARD_RENDER_FINGERPRINT`, committed beside `OG_CARD_CONTRACT_VERSION` for exactly this comparison.

**A D1 migration has to reach the remote database before the deploy that reads it, and nothing enforces the order.** Workers Builds deploys on every push to `main` and applies no migration; `npx wrangler d1 migrations apply ryanlindsey-me-db --remote` is a separate step somebody runs by hand. Merging first means the deployed code selects a column the database does not have, and on `/ops` that is worse than losing one figure: `readOpsMetrics` issues its four statements through `db.batch`, which is one transaction, so a single unknown column rejects the whole batch, `readOrNull` catches it, and the page loses tool calls, chat sessions, chat turns and eval runs together, every one of them rendering as "the metrics store could not be read". `migrations/0005_eval_run_status.sql` is the live case: `/ops` selects `eval_runs.status` as of issue #291, so apply that migration before merging the branch that reads it.
...
`scripts/registry-publish.mjs` publishes this server's entry to the official MCP Registry under the `me.ryanlindsey` namespace. It builds the entry from `src/lib/discovery/registry-entry.ts`; `src/lib/discovery/server-card-v1.ts` reads this same builder for the SEP-2127 card, so the registry and the card cannot disagree and no copy of the version is committed for release-please to bump. The namespace is proved by `/.well-known/mcp-registry-auth`, one line carrying the public key, and the private key reaches the script only through `op run`. It needs `mcp-publisher` installed, which is not a dependency of this repository.
