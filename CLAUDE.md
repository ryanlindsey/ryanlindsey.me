...
That sentence is why this is an S3 pair rather than a Bearer token, and the distinction cost four failed runs to learn (issue 07, #205). The workflow shipped in #185 calling `wrangler r2 object` with `CLOUDFLARE_API_TOKEN`, and never once succeeded. Measured in the runner against the stored secret: `/user/tokens/verify` answered `1000 Invalid API Token`, and R2 answered `10000` for both accounts even on `r2/buckets`, which merely lists, so widening the bucket scope could not have helped and measurably did not. What that proves is that the value CI held was not a valid Cloudflare API token, and no more: the R2 token screen issues several values at once, and which one was stored is not recoverable now. The repair that would have let wrangler work is a Custom API token carrying the `Workers R2 Storage` permission groups, and those are account-wide, so taking it would have spent the guarantee above. The mechanism was copied from `scripts/private-doc.mjs`, which shells out to wrangler and works because it runs behind the owner's OAuth login, an account-level credential. Copying the mechanism without its credential is the whole bug.

Three things follow. Rotation happens in the Cloudflare dashboard and needs no change here, though it now moves two values rather than one. The credentials cannot deploy a Worker, so Workers Builds still owns deploys and still mints its own. And `10 §2.4` in the private docs repo states that CI holds zero Cloudflare credentials, which is now false there as well; correcting it is a separate change in that repository.
