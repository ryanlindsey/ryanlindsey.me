...
That sentence is why this is an S3 pair rather than a Bearer token, and the distinction cost five failed runs to learn (issue 07, #205). The workflow shipped in #185 calling `wrangler r2 object` with `CLOUDFLARE_API_TOKEN`, and never once succeeded. An R2 API token is not a Cloudflare API token: it is an S3 credential for the endpoint above, while `wrangler r2 object` speaks Bearer to `api.cloudflare.com`. Measured in the runner against the stored secret, it failed even on `r2/buckets`, which merely lists, so widening the bucket scope could not have helped and measurably did not. The repair that would have let wrangler work is a Custom API token carrying the `Workers R2 Storage` permission groups, and those are account-wide, so taking it would have spent the guarantee above. The mechanism was copied from `scripts/private-doc.mjs`, which shells out to wrangler and works because it runs behind the owner's OAuth login, an account-level credential. Copying the mechanism without its credential is the whole bug.
